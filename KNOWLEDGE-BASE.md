...
> 维护角色：本会话为 v1 总控/中枢，负责持续整理产出、吃透 v1 代码/数据库/内容，为原型设计等其他会话提供支持。
> 📌 **置顶主线**：所有新增/迁移走 [产品工作流·从需求大纲到最终呈现](产品工作流.md)（需求大纲→归档→原型→评审→开发→验收→呈现）。
> 最后更新：2026-06-18（本次并入 06-18 增量，详见 §10；交接摘要见 `../总控交接.md`）
...
| 文件 | 用途 |
|---|---|
| **saas-workplan.html** | SaaS 后台工作计划书（8板块+三阶段计划+二阶段计划，**最重要的总览**，进度数据权威来源）。06-18 改版：由「迁移对比报告」更名而来，新增 SitINR 原功能/SaaS 已完成/差异化标记，进度以模块下功能点为单位、状态数字可点击跳转，状态统一为 **已完成/进行中/规划中** 三态 |
...
| **member-detail-field-glossary-full.html** | 会员详情 · 全字段定义说明（06-18 随会员列表 38 功能点核对同步更新；仅记会员详情字段，不含活动任务） |
...
| **phase2-member-mgmt.html** | 二阶段会员管理原型（06-18 新建，基于 SaaS UI，含子页面子功能） |
...
> 06-18：计划书状态口径统一为 **已完成 / 进行中 / 规划中** 三态（原“已迁移/待迁移/不迁移/开发中”等并入此三态）；进度按模块下功能点统计，数字可点击跳转介绍页。
...
- 会员端（后台 mock）：`http://localhost:8200/#/profile`（固定端口，admin/123456）
- 会员端原型（beck 复制版，06-18 本地化）：`http://localhost:8753/ar-saas-vue-app.html#/profile`（源 `beck.22889.club/#/profile`，本地确认后再发布）
...
## 10. 增量变更记录 · 2026-06-18（总控会话 6f126728）

本日 7 项产出，已在上文各节就地更新，此处汇总留痕。停在干净状态（运营管理去活动任务后功能点 38→35）。

1. **集团后台权限管理**（`prototypes/group-admin.html`）：系统管理-组织架构下新增「权限管理」；角色权限中集团/商户各只保留唯一内置角色「管理员」；初始权限按模块细化、子项可配置。去掉「市场管理」「商户管理」顶层入口。
2. **组织架构-商户管理 / 渠道管理**：集团视角可创建商户并指定后台账号密码（账号可自填，非默认 admin）、编辑/保存/发布；商户视角新增「渠道管理」生成推广渠道链接。规则配置仅留低频入口（去掉活动/红包/转盘/签到/首充）。创建成功后有「一键复制」分享按钮。
3. **工作计划书改版**（`docs/saas-workplan.html`）：更名自「迁移对比报告」；新增 SitINR 原功能/SaaS 已完成/差异化标记；进度按模块下功能点、状态数字可点击跳转；状态统一为**已完成/进行中/规划中**三态。
4. **二阶段介绍（图文）**：①顶部 tab 定义及 8 模块分类优化 ②快捷工作台新增编辑功能（增/删/排序）③用户详情新增红字归因字段。来源页 `localhost:8200/#/online-users`、`#/member-list`。
5. **会员列表功能点**（`#/member-list`）：核对为 **38 个功能点** → 运营管理去「活动任务」后为 **35**；注册与登录拆开重排；合并「注册设备号+指纹」；新增「同设备/指纹注册用户数与登录数」「登录浏览器指纹+设备号」；`member-detail-field-glossary-full.html` 同步更新。
6. **会员端原型本地化**：复制 `beck.22889.club/#/profile` → `http://localhost:8753/ar-saas-vue-app.html#/profile`，本地确认后再发布。
7. **memory 新增**：`knowledge-base-role.md`（中枢角色）、`saas-prototype-design-principles.md`（设计四原则）。

**承接窗口下一步**：二阶段剩余功能点继续推进（以 `saas-workplan.html` 二阶段计划为准）；对齐 §7 待办与本节 §9 的 6 项待确认。
